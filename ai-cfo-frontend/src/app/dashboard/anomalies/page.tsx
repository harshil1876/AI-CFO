...
export default function AnomalyHubPage() {
  ...
  const fetchAnomalies = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://127.0.0.1:8000/api/anomalies/?bot_id=${botId}`, {
  ...
  const fetchComments = async (anomalyId: number) => {
    try {
      ...
      const res = await fetch(
        `http://127.0.0.1:8000/api/anomalies/${anomalyId}/comments/?bot_id=${botId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (res.ok) {
        const data = await res.json();
  ...
  const updateStatus = async (anomalyId: number, newStatus: string) => {
    try {
      const token = await getToken();
      await fetch(`http://127.0.0.1:8000/api/anomalies/${anomalyId}/status/`, {
        method: "PATCH",
        headers: {
  ...
  const submitComment = async (anomalyId: number) => {
    ...
    try {
      const token = await getToken();
      await fetch(`http://127.0.0.1:8000/api/anomalies/${anomalyId}/comments/`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-amber-500/10 pb-6">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-3 bg-clip-text text-transparent bg-gradient-to-r from-amber-200 to-amber-500">
            ...
            Anomaly Resolution Hub
          </h1>
          <p className="text-slate-400 mt-2">
            Collaborate, assign, and resolve AI-detected financial anomalies.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <input
            ...
            className="bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-xs text-slate-300 placeholder-slate-500 outline-none focus:border-amber-500/50 w-52"
          />
          <button
