...
export default function TeamPermissionsPage() {
  ...
  const fetchPermissions = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/team/permissions/?bot_id=${botId}`, {
        headers: {
  ...
  const togglePermission = async (userId: string, userEmail: string, feature: keyof TeamPermission, currentValue: boolean) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/team/permissions/update/`, {
        method: 'POST',
        headers: {
